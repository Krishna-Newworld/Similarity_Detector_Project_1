...
def extract_features_from_sentence_pair(sent1, sent2):
    ...
    bpt_cosine = cosine_from_texts(sent1[::-1], sent2[::-1])  # placeholder
...
model_dir = "C:/Users/taskf/OneDrive/Desktop/TextSimilarity/saved_models"
label_encoder_path = os.path.join(model_dir, "label_encoder.pkl")
scaler_path = os.path.join(model_dir, "scaler.pkl")

if not os.path.exists(scaler_path) or not os.path.exists(label_encoder_path):
    raise FileNotFoundError("🚫 Missing scaler.pkl or label_encoder.pkl. Retrain models with saving included.")
...
def predict_with_all_models(sent1, sent2):
    ...
    for filename in os.listdir(model_dir):
        if filename.endswith("_model1.pkl"):
            ...
            pred2 = model2.predict(scaled_features)

            final_pred = pred2[0] if model2 else pred1[0]
...
pair_1 = ("There is a cat on the roof.", "There is a dog on the roof.")
pair_2 = ("It rained heavily yesterday A severe storm hit the area.", "A severe storm hit the area.")

print("📌 Predictions for Pair 1:")
for model, label in predict_with_all_models(*pair_1).items():
    print(f"{model}: {label}")

print("\n📌 Predictions for Pair 2:")
for model, label in predict_with_all_models(*pair_2).items():
    print(f"{model}: {label}")
